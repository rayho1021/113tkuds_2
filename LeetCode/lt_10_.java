...
class Solution {
   public boolean isMatch(String s, String p) {
       ...
       // dp[i][j] 表示 s 的前 i 個字元是否匹配 p 的前 j 個字元
       ...
       for (int i = 1; i <= m; i++) {
           for (int j = 1; j <= n; j++) {
               ...
               if (pc == '*') {
                   ...
                   // 處理一般字元或 '.' 的情況
                   if (pc == '.' || pc == sc) {
                       dp[i][j] = dp[i - 1][j - 1]; // 字元匹配，檢查前面的狀態
                   }
                   // 如果字元不匹配，dp[i][j] 保持 false（預設值）
               }
           }
       }
...
4. '*' 的關鍵理解：它修飾前面的字元，可以讓該字元出現 0 次、1 次或多次。
5. 邊界處理：注意陣列索引與字串索引的對應關係（dp 多一行一列）。
6. 時間複雜度 O(m×n)，空間複雜度 O(m×n)，其中 m 和 n 分別是字串和模式的長度。
*/