...
public class LC40_CombinationSum2_Procurement {
    public static void main(String[] args) {
        ...
        Arrays.sort(candidates); // 排序便於升序、去重
    ...
    private static void backtrack(int[] candidates, int remain, int start,
        ...
        for (int i = start; i < candidates.length; i++) {
            ...
            if (i > start && candidates[i] == candidates[i - 1]) continue; // 去重：同層跳過相同數

            path.add(candidates[i]);
            backtrack(candidates, remain - candidates[i], i + 1, path, res); // 只能用一次 → 傳 i+1
